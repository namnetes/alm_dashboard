...
help:
	...
	@printf "  $(BOLD)homer-stop$(RESET)     Arrête et supprime le conteneur Homer\n"
...
homer-start:
	...
	@if docker ps -a --format '{{.Names}}' | grep -q "^$(HOMER_NAME)$$"; then \
		printf "$(YELLOW)Attention$(RESET) : le conteneur '$(HOMER_NAME)' existe déjà.\n"; \
		...
		docker run -d \
			...
			$(HOMER_IMAGE) >/dev/null || { \
				printf "$(RED)Erreur$(RESET) : échec du démarrage du conteneur.\n"; exit 1; \
			}; \
		sleep 1; \
		docker ps --filter "name=^$(HOMER_NAME)$$" --format '{{.Names}}' \
			| grep -q "^$(HOMER_NAME)$$" || { \
				printf "$(RED)Erreur$(RESET) : le conteneur s'est arrêté immédiatement après le démarrage.\n"; \
				printf "  → Consultez les logs : docker logs $(HOMER_NAME)\n"; \
				exit 1; \
			}; \
		printf "$(GREEN)OK$(RESET) Homer démarré → http://127.0.0.1:$(HOMER_PORT)\n"; \
	fi
...
homer-stop:
	...
	$(call check_docker_daemon)
	@if ! docker ps -a --format '{{.Names}}' | grep -q "^$(HOMER_NAME)$$"; then \
		printf "$(YELLOW)Attention$(RESET) : aucun conteneur '$(HOMER_NAME)' trouvé — rien à faire.\n"; \
		exit 0; \
	fi
...
homer-logs:
	...
	@docker ps --filter "name=^$(HOMER_NAME)$$" --format '{{.Names}}' \
		| grep -q "^$(HOMER_NAME)$$" || { \
			printf "$(YELLOW)Attention$(RESET) : '$(HOMER_NAME)' existe mais n'est pas en cours d'exécution.\n"; \
